...
"""
function hostSpectrum(nFFT,nbTime);
	# --- Update USRP config.
	global doTask	= 1;
	global yLim	    = (-40,40);
	global changed  = 0;
	...
	while(true)
		...
		sF		= sF ./ nbSegMean;
		# --- Forget older acquisition and transform with new
		sFAll	   = circshift(sFAll,(0,1));
		sFAll[:,1] = sF;
		# --- Configuration axis 
		if changed == 1
			xAx     = ((collect(0:nFFT-1) ./ nFFT) .-0.5) .* round(radio.samplingRate,digits=2);
		...
		# --- Update plot 
		plt = heatmap(transpose(sFAll),aspect_ratio=1);
